...
exports.approveEvent = async (req, res) => {
  try {
    ...
    if (event.zone !== req.user.zone) {
      return res.status(403).json({
        message: "No puedes validar eventos fuera de tu zona"
      });
    }
...
exports.recategorizeEvent = async (req, res) => {
  try {
    ...
    if (event.zone !== req.user.zone) {
      return res.status(403).json({
        message: "No puedes recategorizar eventos fuera de tu zona"
      });
    }
...
exports.mergeDuplicateEvents = async (req, res) => {
  try {
    ...
    if (primaryEvent.zone !== req.user.zone || duplicateEvent.zone !== req.user.zone) {
      return res.status(403).json({
        message: "No puedes fusionar eventos fuera de tu zona"
      });
    }
    ...
    await AuditLog.create({
      action: "MERGE_DUPLICATE_EVENTS",
      entityType: "Event",
      entityId: duplicateEvent._id,
      performedBy: req.user._id,
      zone: req.user.zone,
      details: {
        primaryEventId,
...
exports.findPossibleDuplicates = async (req, res) => {
  try {
    ...
    if (zone && zone === req.user.zone) filters.zone = zone;
